  ...
  if (networks.length > 0) {
    for (const netConfig of networks) {
      switch (netConfig.protocolVersion) {
        case LEGACY: {
          ServiceFactory.register(`feed-${netConfig.network}`, (serviceName) => new LegacyFeedClient(apiEndpoint, serviceName.slice(5)));
          break;
        }
        case CHRYSALIS: {
          ServiceFactory.register(`feed-${netConfig.network}`, (serviceName) => new ChrysalisFeedClient(apiEndpoint, serviceName.slice(5)));
          break;
        }
        case STARDUST: {
          ServiceFactory.register(`feed-${netConfig.network}`, (serviceName) => new StardustFeedClient(apiEndpoint, serviceName.slice(5)));
          break;
        }
        default:
      }
