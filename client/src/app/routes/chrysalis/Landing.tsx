...
class Landing extends Feeds<RouteComponentProps<LandingRouteProps>, LandingState> {
  ...
  public async componentDidMount(): Promise<void> {
    ...
    this.setState({
      ...
      valuesFilter: filterSettings?.valuesFilter ?? getFilterFieldDefaults(this._networkConfig?.protocolVersion ?? CHRYSALIS),
  ...
  public render(): ReactNode {
                ...
                {this.state.networkConfig.isEnabled && (
                  <div className="row space-between info-boxes">
                    <div className="info-box">
                      <span className="info-box--title">Messages per sec</span>
                      <div className="info-box--value">
                        <span className="download-rate">{NumberHelper.roundTo(Number(this.state.itemsPerSecond), 1) || "0"}</span>
                      </div>
                    </div>
                    <div className="info-box">
            ...
            <div className="inner">
              <div className="card margin-t-m">
                <div className="card--value card--value__no-margin description col row middle" style={{ whiteSpace: "nowrap" }}>
                  ...
                  <p>
                    <span>It can only be used to browse historic data before milestone 7669900</span>
                  </p>
                </div>
              </div>
              {!this.state.networkConfig.isEnabled && (
    ...
    const isLatestMilestoneFeedInfoEnabled = this._networkConfig && this._networkConfig.network !== CUSTOM;
              ...
              {this.state.networkConfig.isEnabled && (
                <div className="row space-between info-boxes">
                  <div className="info-box">
                    <span className="info-box--title">Messages per sec</span>
                    <div className="info-box--value">
                      <span className="download-rate">{NumberHelper.roundTo(Number(this.state.itemsPerSecond), 1) || "--"}</span>
                    </div>
                  </div>
                  <div className="info-box">
                    <span className="info-box--title">Inclusion rate</span>
                  ...
                  <div className="section--header row space-between padding-l-8">
                    ...
                    <div className="feed--actions">
                      ...
                      <div className="filters-button-wrapper">
                        ...
                        <div className="filters-button-wrapper__counter">{this.state.valuesFilter.filter((f) => f.isEnabled).length}</div>
                      </div>
                      {this.state.isFilterExpanded && (
                        <div className="filter-wrapper">
                          <div className="filter">
                            <div className="filter-header row space-between middle">
                              <button className="button--unstyled" type="button" onClick={() => this.resetFilters()}>
                                Reset
                              </button>
                            ...
                            <div className="filter-content">
                              {this.state.valuesFilter.map((payload) => (
                                <React.Fragment key={payload.label}>
                                  <label>
                                    <input type="checkbox" checked={payload.isEnabled} onChange={() => this.toggleFilter(payload.label)} />
                                    {payload.label}
                                  </label>
                    ...
                    {this.state.filteredItems.map((item) => (
                      <div className="feed-item" key={item.id}>
                        <div className="feed-item__content">
                          ...
                          <Link className="feed-item--hash" to={RouteBuilder.buildItem(this.state.networkConfig, item.id)}>
                            {item.id}
                          </Link>
                        </div>
                        <div className="feed-item__content">
              ...
              <div className="card margin-t-m">
                ...
                {this.state.networkConfig.faucet && (
                  <div className="card--content description">
                    ...
                    <a className="data-link margin-l-t" href={this.state.networkConfig.faucet} target="_blank" rel="noopener noreferrer">
                      {this.state.networkConfig.faucet}
                    </a>
                  </div>
                )}
      ...
      this.setState({
        marketCapCurrency:
          this._currencyData.coinStats?.iota?.marketCap ?
            this._currencyService.convertFiatBase(this._currencyData.coinStats.iota.marketCap, this._currencyData, true, 2, undefined, true)
          : "--",
        priceCurrency:
      ...
      const filters = [
        ...
        },
        {
          ...
          filter: (item: IFeedItem) =>
            item.value !== undefined && item.value !== 0 && Math.abs(item.value) >= minLimit && Math.abs(item.value) <= maxLimit,
        },
        {
  ...
  private transactionDropdown(type: "minimum" | "maximum"): ReactNode {
    return (
      <div className="col">
        <span className="label margin-b-2">{type === "minimum" ? "Min value" : "Max value"}</span>
        <span className="filter--value">
          <input
            className="input-plus"
            type="text"
            value={type === "minimum" ? this.state.valueMinimum : this.state.valueMaximum}
            onChange={(e) => (type === "minimum" ? this.updateMinimum(e.target.value) : this.updateMaximum(e.target.value))}
          />
