                  ...
                  <div className="card--header card--header__space-between">
                    <h2>
                      ...
                      {this.state.details && this.state.milestoneIndex === undefined && (
                        <Confirmation state={this.state.details.confirmationState} milestoneIndex={this.state.details.milestoneIndex} />
                      )}
                      {this.state.milestoneIndex !== undefined && (
                        <div className="card--header-count card--header-count__success">Milestone Index {this.state.milestoneIndex}</div>
                      )}
                    </h2>
                    {this.state.details && (
                      <div className="h1-sub">{DateHelper.format(DateHelper.milliseconds(this.state.details.tx.timestamp))}</div>
                    )}
                  </div>
                  <div className="card--content">
                    ...
                    {this.state.details && (
                      <React.Fragment>
                        ...
                        <div className="card--value row middle">
                          <Link to={`/${this.props.match.params.network}/address/${this.state.address}`} className="margin-r-t">
                            {this.state.address}
                  ...
                  <React.Fragment>
                    <div className="card">
                            ...
                            className={classNames(
                              "card--header-count",
                              {
                                "card--header-count__error":
                                  this.state.isBundleValid === "invalid" || this.state.isBundleValid === "consistency",
                              },
                              {
                      ...
                      <div className="card--content">
                        ...
                        <div className="card--value row middle">
                          <Link to={`/${this.props.match.params.network}/bundle/${this.state.details?.tx.bundle}`} className="margin-r-t">
                      ...
                      <div className="card--content">
                        <div className="row row--tablet-responsive">
                          <div className="col fill">
                            ...
                            <div className="card--value row middle">
                              <Link to={`/${this.props.match.params.network}/tag/${this.state.details?.tx.tag}`} className="margin-r-t">
                        ...
                        {this.state.streamsV0Root && (
                          <div className="col fill">
                            ...
                            <div className="card--value row middle">
                              <Link to={`/${this.props.match.params.network}/streams/0/${this.state.streamsV0Root}`} className="margin-r-t">
                                {this.state.streamsV0Root}
                              </Link>
                            </div>
                          </div>
                        ...
                        {this.state.message !== undefined && (
                          <React.Fragment>
                            <div className="card--label row middle margin-b-2">
                              <span className="margin-r-s">
                                {this.state.messageType !== "Trytes" && (
                                  ...
                                  >
                                    Message {this.state.showRawMessageTrytes ? "Trytes" : this.state.messageType}
                                  </button>
                                )}
                                {this.state.messageType === "Trytes" && `Message ${this.state.messageType}`}
                              </span>
                              <CopyButton copy={this.state.showRawMessageTrytes ? this.state.rawMessageTrytes : this.state.message} />
                            </div>
                            <div
                              className={classNames(
                                "card--value",
                            ...
                            {this.state.messageSpan && (
                              <div className="card--value">The content of this message spans multiple transactions.</div>
                            )}
                          </React.Fragment>
                        )}
                      </div>
                      ...
                      <div className="col fill">
                        <div className="card">
                          ...
                          <div className="card--content">
                            ...
                            <div className="card--value">{this.state.details.tx.attachmentTimestampLowerBound}</div>
                            <div className="card--label">Attachment Upper Bound</div>
                            <div className="card--value">{this.state.details.tx.attachmentTimestampUpperBound}</div>
                            {this.state.details.tx.attachmentTimestamp !== 0 && (
                              <React.Fragment>
                                <div className="card--label">Attachment Timestamp</div>
                                <div className="card--value">
                                  {DateHelper.format(DateHelper.milliseconds(this.state.details.tx.attachmentTimestamp))}
                                </div>
                              </React.Fragment>
                            )}
                          </div>
                      ...
                      <div className="col fill">
                        <div className="card">
                          ...
                          <div className="card--content">
                            ...
                            <div className={classNames("card--value", "card--value-textarea", "card--value-textarea__tall")}>
                              {this.state.raw}
                            </div>
                          </div>
                        </div>
                    ...
                    <div className="card margin-t-s">
                      <div className="card--header">
                        <h2>Children</h2>
                        {this.state.children !== undefined && <span className="card--header-count">{this.state.children.length}</span>}
                      </div>
      ...
      this.setState(
        ...
        async () => {
          if (this.state.details) {
            const thisGroup = await this._tangleCacheService.getTransactionBundleGroup(this.props.match.params.network, this.state.details);
            ...
            if (thisGroup.length > 0) {
              ...
              const isBundleValid = this.state.details.confirmationState === "confirmed" ? true : isBundle(thisGroup.map((t) => t.tx));
              let total = 0;
