...
export const getPages = (
    ...
    if (networks.length > 0 && currentNetwork !== undefined) {
        ...
        pages.push({
            label: "Visualizer",
            url: `/${currentNetwork.network}/visualizer/`
        });

        if (currentNetwork.hasStatisticsSupport) {
            pages.push({
                label: "Statistics",
                url: `/${currentNetwork.network}/statistics/`
            });
        }
    }
...
export const buildUtilities = (
    ...
    if (networks.length > 0) {
        utilities.push({
            label: "Streams v0",
            url: `/${currentNetwork}/streams/0/`
        });
        if (identityResolverEnabled) {
            utilities.push({
                label: "Decentralized Identifier",
                url: `/${currentNetwork}/identity-resolver/`
            });
        }
    }
...
export const getFaviconHelmet = (isShimmer: boolean) => {
    ...
    return (
        <Helmet>
            <link
                rel="shortcut icon"
                href={`${publicUrl}/favicon/${folder}/favicon.ico`}
                data-react-helmet="true"
            />
            <link
                rel="manifest"
                href={`${publicUrl}/favicon/${folder}/site.webmanifest`}
                data-react-helmet="true"
            />
            <link
                rel="apple-touch-icon"
                sizes="180x180"
                href={`${publicUrl}/favicon/${folder}/favicon-180x180.png`}
                data-react-helmet="true"
            />
            <link
                rel="icon"
                type="image/png"
                sizes="32x32"
                href={`${publicUrl}/favicon/${folder}/favicon-32x32.png`}
                data-react-helmet="true"
            />
            <link
                rel="icon"
