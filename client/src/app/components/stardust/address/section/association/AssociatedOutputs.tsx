  ...
  return associations.length === 0 ?
    ...
    : <div className="section associated-outputs">
        ...
        {associationTypesToRender?.map((associationType, idx) => {
          const targetAssociation: IAssociation | undefined = associations.find((association) => association.type === associationType);
          return <AssociationSection key={`${currentTab}-${idx}`} association={associationType} outputIds={targetAssociation?.outputIds} />;
        })}
      </div>;
};
