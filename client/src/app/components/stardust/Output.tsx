...
class Output extends Component<OutputProps, OutputState> {
  ...
  public render(): ReactNode {
    ...
    const outputIdTransactionPart =
      displayFullOutputId ? `${outputId.slice(0, -4)}` : `${outputId.slice(0, 8)}....${outputId.slice(-8, -4)}`;
          ...
          <div className="output padding-l-t left-border">
            ...
            {output.type === OutputType.Foundry && (
              <React.Fragment>
                ...
                <div className="card--value">
                  {foundryId && (
                    <TruncatedId id={foundryId} link={isLinksDisabled ? undefined : `/${network}/foundry/${foundryId}`} showCopyButton />
                  )}
                </div>
