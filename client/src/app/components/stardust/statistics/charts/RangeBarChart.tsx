...
export const RangeBarChart: React.FC<IRangeBarChartProps> = ({ data, yField, yLabel }) => {
  ...
  useLayoutEffect(() => {
    if (data && data.length > 0 && wrapperWidth && wrapperHeight && subunitThreshold) {
      ...
      // data dependent vars
      const dataY = yField === "addressCount" ? data.map((d) => Number(d[yField])) : data.map((d) => Number(d[yField]) / subunitThreshold);
      ...
      svg
        ...
        .attr("rx", 2)
        .attr("height", (d) => INNER_HEIGHT - y(yField === "addressCount" ? Number(d[yField]) : Number(d[yField]) / subunitThreshold))
