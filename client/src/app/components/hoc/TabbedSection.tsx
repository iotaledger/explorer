  ...
  const tabsView = (
    <div className="tabbed-section--tabs-wrapper">
      {TABS.map((tab, idx) => {
        const isDisabled =
          tabOptions ?
            tabOptions[tab]?.disabled === undefined ?
              false
            : tabOptions[tab].disabled
          : false;

        const counter =
          tabOptions ?
            tabOptions[tab]?.counter === undefined ?
              0
            : tabOptions[tab].counter
          : 0;

        const isLoading =
          tabOptions ?
            tabOptions[tab]?.isLoading === undefined ?
              false
            : tabOptions[tab].isLoading
          : false;
        ...
        const hidden =
          tabOptions ?
            tabOptions[tab]?.hidden === undefined ?
              false
            : tabOptions[tab].hidden
          : false;
