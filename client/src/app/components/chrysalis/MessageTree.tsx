...
class MessageTree extends Component<MessageTreeProps, MessageTreeState> {
    ...
    return (
      <React.Fragment>
        ...
        >
          ...
          {/* Edges */}
          <svg className="edge">
            {this.state.edges?.map((edge) => <line key={edge.id} x1={edge.x1} x2={edge.x2} y1={edge.y1} y2={edge.y2} stroke="black" />)}
          </svg>
        </div>
      </React.Fragment>
    );
  ...
  private loadItemsUI(): void {
    const parentsHeight =
      (this.state.config.itemHeight + this.state.config.verticalSpace) * this.props.parentsIds.length - this.state.config.verticalSpace;
    const childrenHeight =
      (this.state.config.itemHeight + this.state.config.verticalSpace) * this.props.childrenIds.length - this.state.config.verticalSpace;
