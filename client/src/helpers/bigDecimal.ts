export default class BigDecimal {
  ...
  constructor(value: string, decimals = 2, rounded = false) {
    ...
    this.n =
      BigInt(whole + fraction.padEnd(this.decimals, "0").slice(0, this.decimals)) + BigInt(this.rounded && fraction[this.decimals] >= "5");
  }
  ...
  public divRound(dividend: bigint, divisor: bigint): BigDecimal {
    return BigDecimal.fromBigInt(dividend / divisor + (this.rounded ? ((dividend * 2n) / divisor) % 2n : 0n), this.decimals, this.rounded);
  }
