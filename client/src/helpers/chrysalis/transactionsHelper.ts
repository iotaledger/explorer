...
export class TransactionsHelper {
        ...
        if (transactionMessage?.payload?.type === TRANSACTION_PAYLOAD_TYPE) {
            ...
            for (let i = 0; i < transactionMessage.payload.unlockBlocks.length; i++) {
                if (transactionMessage.payload.unlockBlocks[i].type === SIGNATURE_UNLOCK_BLOCK_TYPE) {
                    ...
                    signatureBlocks.push(transactionMessage.payload.unlockBlocks[refUnlockBlock.reference] as ISignatureUnlockBlock);
                }
            }
            ...
            for (let i = 0; i < transactionMessage.payload.essence.inputs.length; i++) {
                ...
                const transactionResult = await tangleCacheService.search(network, input.transactionId);
                let amount = 0;
                if (transactionResult?.message && transactionResult?.message.payload?.type === TRANSACTION_PAYLOAD_TYPE) {
            ...
            for (let i = 0; i < transactionMessage.payload.essence.outputs.length; i++) {
                if (
                    transactionMessage.payload.essence.outputs[i].type === SIG_LOCKED_SINGLE_OUTPUT_TYPE ||
            ...
            for (let i = 0; i < inputs.length; i++) {
                const outputResponse = await tangleCacheService.outputDetails(network, inputs[i].outputHash);
    ...
    public static async getMessageStatus(
        ...
        let date: string = "";
        const details = await tangleCacheService.messageDetails(network, messageId ?? "");
        if (details) {
            if (details?.metadata) {
            ...
            if (milestoneIndex) {
                const result = await tangleCacheService.milestoneDetails(network, milestoneIndex);
                if (result?.timestamp) {
