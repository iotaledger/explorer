...
export abstract class InfluxDbClient {
  ...
  private async collectAnalytics() {
    ...
    try {
      ...
      for (const update of await this.queryInflux<ITimedEntry & { nftsCount: string }>(NFT_STAT_TOTAL_QUERY, null, this.getToNanoDate())) {
      ...
      .then((results) => {
        for (const update of results) {
          if (this.isAnyFieldNotNull<T>(update)) {
            if (debug) {
              logger.debug(`[InfluxDb] Setting ${description} cache entry (${network}):`, moment(update.time).format(DAY_KEY_FORMAT));
            }
