...
export class Converter {
  ...
  public static bytesToUtf8(array: ArrayLike<number>, startIndex?: number, length?: number | undefined): string {
    ...
    for (let i = start; i < start + len; i++) {
      ...
      if (value < 0x80) {
        ...
        // surrogate pair
        const charCode =
          (((value & 0x07) << 18) | ((array[i + 1] & 0x3f) << 12) | ((array[i + 2] & 0x3f) << 6) | (array[i + 3] & 0x3f)) - 0x010000;
  ...
  public static hexToBytes(hex: string, reverse?: boolean): Uint8Array {
    ...
    if (Converter.DECODE_LOOKUP) {
      ...
      let n = 0;
      while (i < length) {
        array[n++] = (Converter.DECODE_LOOKUP[strippedHex.charCodeAt(i++)] << 4) | Converter.DECODE_LOOKUP[strippedHex.charCodeAt(i++)];
      }
